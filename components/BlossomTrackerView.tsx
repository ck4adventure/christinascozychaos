...
import { Task, TaskLog, TaskWithStatus, Frequency } from '@/types';
...
interface BlossomTrackerViewProps {
  tasks: Task[];
  logs: TaskLog[];
  tasksWithStatus: TaskWithStatus[];
  addTask: (data: Omit<Task, 'id' | 'createdAt' | 'flower'>) => Promise<Task>;
  addOneOffTask: (data: Omit<Task, 'id' | 'createdAt' | 'flower' | 'frequency'>) => Promise<Task>;
  toggleTask: (taskId: string) => Promise<void>;
  toggleTaskForDate: (taskId: string, date: Date) => Promise<void>;
  deleteTask: (taskId: string) => Promise<void>;
  editTask: (taskId: string, updates: Partial<Omit<Task, 'id' | 'createdAt' | 'flower'>>) => Promise<void>;
  demo?: boolean;
  resetDemo?: () => void;
}

/** Pure presentational tracker UI — data source (real API vs. browser-only demo) is injected by the caller. */
export default function BlossomTrackerView({
  tasks,
  logs,
  tasksWithStatus,
  addTask,
  ...
  return (
    <div className="blossom-page">
      ...
      <div className="blossom-inner">
        <Link href="/" className="btn btn--link mobile-nav-link" style={{ marginBottom: demo ? '1rem' : '2.5rem' }}>← Home</Link>

        {demo && (
          <div className="chip" style={{ display: 'inline-flex', gap: '0.5rem', marginBottom: '1.5rem', cursor: 'default' }}>
            🔒 Demo — stored only in this browser
            {resetDemo && (
              <button
                onClick={resetDemo}
                className="btn--link"
      ...
      <NavBar active={tab} onChange={handleTabChange} demo={demo} />
